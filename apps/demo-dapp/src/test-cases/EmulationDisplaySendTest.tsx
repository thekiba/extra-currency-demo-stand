...
/**
 * Default test steps for EC transfer emulation display test
 */
const DEFAULT_STEPS: TestStep[] = [
  {
    id: STEP_ID.CHECK_EMULATION,
    title: 'Emulation Display',
    description: 'Verify that your wallet correctly displays the transaction preview for outgoing EC transfer',
    type: 'user-action',
    status: 'pending'
  }
];
...
/**
 * Component for testing whether wallet correctly displays EC transfer transaction preview
 */
export function EmulationDisplaySendTest(props: BaseTestCaseProps) {
  const { 
    steps, 
    updateStepDetails,
    setStepSuccess,
    ...
    <Alert>
      <AlertDescription className="space-y-2">
        ...
        <ul className="list-disc pl-4 space-y-1 text-sm">
          ...
          <li>The correct recipient address (EC swap contract)</li>
          <li>All transaction details and fees</li>
        </ul>
        <p className="text-sm text-muted-foreground mt-2">
          Note: The transaction preview should clearly show all details of the outgoing EC transfer.
        </p>
      </AlertDescription>
    </Alert>
  );
  ...
  const renderSuccessMessage = (): ReactNode => (
    <div className="text-sm text-green-600">
      ✓ Transaction preview displays correct outgoing transfer details
    </div>
  );
      ...
      <AlertDescription className="space-y-2">
        ...
        <div className="mt-2 text-sm bg-red-500/10 p-4 rounded space-y-2">
          ...
          <ul className="list-disc pl-4 space-y-1">
            ...
            <li>The complete recipient address (EC swap contract)</li>
            <li>All transaction details including fees and warnings</li>
          </ul>
        </div>
      </AlertDescription>
    </Alert>
